...
export class CommentService {
  ...
  async findAll(taskId: string, userId: string) {
    await this.ensureTaskOwner(taskId, userId);
    return prisma.comment.findMany({ where: { taskId }, orderBy: [{ createdAt: 'asc' }, { id: 'asc' }] });
  }
  ...
  async delete(id: string, taskId: string, userId: string) {
    ...
    if (result.count === 0) throw new AppError('Comentário não encontrado', 404, 'COMMENT_NOT_FOUND');
  }
}
